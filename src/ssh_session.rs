...
impl SSHSession {
	...
	pub fn set_host(&self, host: &str) -> Result<(),()> {
		...
		let opt = libssh::ssh_options_e::SSH_OPTIONS_HOST as u32;
		let res = host.with_c_str(|h| {
			unsafe { libssh::ssh_options_set(self._session, opt, h as *const c_void) }
		});
	...
	pub fn connect(&self, verify_public_key: |remote_public_key: &ssh_key::SSHKey| -> bool)
			-> Result<(), String>
	{
		assert!(self._session.is_not_null());

		let res = unsafe { libssh::ssh_connect(self._session) };
		if res != libssh::SSH_OK {
			let ptr = self._session as *mut c_void;

			let err_msg = unsafe {
				let err = libssh::ssh_get_error(ptr);
		...
		let remote_public_key = try!(
			ssh_key::SSHKey::from_session(self).map_err(|err| err.to_string())
		);
	...
	pub fn auth_by_public_key(&self, username: Option<&str>, pubkey: &ssh_key::SSHKey)
		-> Result<(),libssh::ssh_auth_e>
	{
		/*
		...
		*/
		...
		let res = libssh::ssh_auth_e::from_i32(ires);
		match res {
			libssh::ssh_auth_e::SSH_AUTH_SUCCESS => Ok(()),
			libssh::ssh_auth_e::SSH_AUTH_PARTIAL |
			libssh::ssh_auth_e::SSH_AUTH_DENIED |
			libssh::ssh_auth_e::SSH_AUTH_AGAIN |
			libssh::ssh_auth_e::SSH_AUTH_ERROR => Err(res),
			x => {panic!("{}", x);}
		}
	}
	...
	pub fn set_port(&self, port: &str) -> Result<(),&'static str> {
		...
		let opt = libssh::ssh_options_e::SSH_OPTIONS_PORT as u32;
		let res = port.with_c_str(|p| unsafe {
			libssh::ssh_options_set(self._session, opt, p as *const c_void)
		});
	...
	pub fn auth_with_public_key<'a>(&self, verify_public_key: |&ssh_key::SSHKey| -> bool)
			-> Result<(),&'a str>
	{
		...
		for _  in range(0, MAX_ATTEMPTS) {
			let msg = try!(ssh_message::SSHMessage::from_session(self));
			...
			match (type_, subtype) {
				...
				{
					let remote_public_key = try!(ssh_key::SSHKey::from_message(&msg));
